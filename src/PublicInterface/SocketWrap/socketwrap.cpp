...
#endif  
	hs = socket(PF_INET, tcpudp, protocol);
	return hs;
}
...
void SocketSendTo(HSocket hs, const char *ptr, int nbytes, transresult_t &rt, sockaddr_in remoteAddr)
{
	rt.nbytes = 0;
	rt.nresult = 0;
	...
	//Linux: flag can be MSG_DONTWAIT, MSG_WAITALL, 使用MSG_WAITALL的时候, socket 必须是处于阻塞模式下，否则WAITALL不能起作用  
	rt.nbytes = sendto(hs, ptr, nbytes, NONBLOCKREADWRITE, (sockaddr*)&remoteAddr, nAddrLen);
	if (rt.nbytes>0)
	{
		rt.nresult = (rt.nbytes == nbytes) ? 0 : 1;
	}
...
void SocketRecvFrom(HSocket hs, char *ptr, int nbytes, transresult_t &rt)
{
	rt.nbytes = 0;
	rt.nresult = 0;
...
int SocketReuseAddr(HSocket hs)
{
	int rt = -1;
	if (ISSOCKHANDLE(hs))
	{
		...
		int opt_val = 1;
			...
			rt = setsockopt(hs, SOL_SOCKET, SO_REUSEADDR, (const char*)&opt_val, sizeof(opt_val)) == 0 ? 0 : 0x1;
...
int CSockWrap::SetReuseAddr()
{	
	return SocketReuseAddr(m_hSocket);
}
...
transresult_t CSockWrap::SendTo(void *ptr, int nbytes)
{
	transresult_t rt;
	SocketSendTo(m_hSocket, (const char *)ptr, nbytes, rt, m_stAddr);
	return rt;
}
transresult_t CSockWrap::RecvFrom(void *ptr, int nbytes)
{
	transresult_t rt;
	SocketRecvFrom(m_hSocket, (char *)ptr, nbytes, rt);
	return rt;
}
